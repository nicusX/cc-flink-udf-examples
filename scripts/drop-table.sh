#!/usr/bin/env bash
# drop-table.sh — Drop a Flink table from Confluent Cloud
#
# Usage:
#   drop-table.sh --table <table-name> [--quiet]
...
usage="Usage: $0 --table <table-name> [--quiet] [--environment-id <id>] [--compute-pool-id <id>] [--database <database>] [--catalog <catalog>]"
...
log "Dropping table '${TABLE_NAME}' from database '${CONFLUENT_FLINK_DATABASE}' ..."
if [[ "${QUIET}" == true ]]; then
